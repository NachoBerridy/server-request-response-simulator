...
def graficador(df, estadisticas, NUM_SERVERS, LAMBDA, MU, CANTIDAD_CLIENTES):
    ...
    with pd.ExcelWriter(name, engine='xlsxwriter') as writer:
        ...
        imgdata.seek(0)
        worksheet = writer.sheets['Datos de los servidores']
        worksheet.insert_image('H2', '', {'image_data': imgdata})
        ...
        plt.figure(figsize=(12, 6))
        plt.hist(tiempos_de_llegada, bins=30, density=True, alpha=0.6, color='b', label='Datos observados')
        ...
        plt.savefig(imgdata, format='png')
        imgdata.seek(0)
        worksheet.insert_image('H22', '', {'image_data': imgdata})
        ...
        plt.savefig(imgdata, format='png')
        imgdata.seek(0)
        worksheet.insert_image('H42', '', {'image_data': imgdata})

        # ---- Gráfica de Tiempo de Ocio por Servidor ----#
        for i in range(NUM_SERVERS):
            plt.figure(figsize=(12, 6))
            plt.hist(df_servidores[i]["Tiempo de ocio"], bins=30, alpha=0.6, label=f"Servidor {i+1}")
            plt.xlabel("Tiempo de ocio")
            plt.ylabel("Densidad")
            plt.title(f'Distribución de los tiempos de ocio por servidor {i+1}')
            plt.legend()
            # Guardar la gráfica en un buffer
            imgdata = io.BytesIO()
            plt.savefig(imgdata, format='png')
            imgdata.seek(0)
            worksheet.insert_image(f'H{62*(i+1)}', '', {'image_data': imgdata})
        ...
        plt.savefig(imgdata, format='png')
        imgdata.seek(0)
        worksheet.insert_image('H82', '', {'image_data': imgdata})
        ...
        plt.savefig(imgdata, format='png')
        imgdata.seek(0)
        worksheet.insert_image('H200', '', {'image_data': imgdata})
